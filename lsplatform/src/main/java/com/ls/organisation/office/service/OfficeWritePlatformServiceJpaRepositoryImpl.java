 ...
 * under the License.
 */
package com.ls.organisation.office.service;
...
import com.ls.organisation.office.domain.OfficeTransaction;
import com.ls.organisation.office.domain.OfficeTransactionRepository;
import com.ls.organisation.office.serialization.OfficeCommandFromApiJsonDeserializer;
import com.ls.organisation.office.serialization.OfficeTransactionCommandFromApiJsonDeserializer;
...
public class OfficeWritePlatformServiceJpaRepositoryImpl implements OfficeWritePlatformService {
    ...
    @Override
    @Caching(evict = {
            @CacheEvict(value = "offices", key = "T(com.ls.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'of')"),
            @CacheEvict(value = "officesForDropdown", key = "T(com.ls.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'ofd')") })
    ...
    @Caching(evict = {
            @CacheEvict(value = "offices", key = "T(com.ls.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'of')"),
            @CacheEvict(value = "officesForDropdown", key = "T(com.ls.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'ofd')"),
            @CacheEvict(value = "officesById", key = "T(com.ls.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#officeId)") })
